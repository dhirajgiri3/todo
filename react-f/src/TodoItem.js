...
function TodoItem({ todo, update }) {
  ...
  let create = new Date(todo.attributes.createdAt).toLocaleString();
  let updated = new Date(todo.attributes.updatedAt).toLocaleString();
  ...
  return (
    <div className="todo">
      ...
      {!edit ? (
        <div className="info">
          <div className="titles">{todo.attributes.item}</div>
          <div className="name">Created At: {create}</div>
          <div className="name">Updated At: {updated}</div>
        </div>
      ) : (
        <form onSubmit={changeTodo}>
          <input
            ...
            type="text"
            placeholder={todo.attributes.item}
            value={newTodo}
